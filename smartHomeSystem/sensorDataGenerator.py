...
conn = sqlite3.connect("smart_home_mgmt.db")
cursor = conn.cursor()
cursor.execute('''CREATE TABLE IF NOT EXISTS energyUsage (id INTEGER PRIMARY KEY AUTOINCREMENT, 
                    DeviceId INTEGER,
...
def SensorDataGenerator(name,deviceid,timestamp=datetime.now()):
    ...
    cursor.execute('''INSERT INTO energyUsage (SensorName, DeviceId, PowerConsumed, timestamp) VALUES (?, ?, ?, ?)''', (name, deviceid, powerConsumed, timestamp))
    conn.commit()
